<footer class="footer">
    <div class="container">
        <div class="row">
            <div class="col-sm-6 boxfot"><img src="{{asset('assets/images/logo.svg')}}" alt="" title="">
                <p class="text">{{__('website.footer.desc')}}</p>
            </div>
            <div class="col-sm-2 boxfot">
                <h6 class="title"> {{__('website.footer.branches.title')}}</h6>
                <nav class="navmenu"> <a href="#"> {{__('website.footer.branches.link-1')}}</a><a href="#"> {{__('website.footer.branches.link-2')}}</a><a href="#"> {{__('website.footer.branches.link-3')}}</a><a href="#">{{__('website.footer.branches.link-4')}}</a><a href="#">{{__('website.footer.branches.link-5')}}</a><a href="#">{{__('website.footer.branches.link-6')}}</a></nav>
            </div>
            <div class="col-sm-2 boxfot">
                <h6 class="title">{{__('website.footer.quick-links.title')}}</h6>
                <nav class="navmenu">
                    <a href="{{route('website.home')}}"> {{__('website.footer.quick-links.link-1')}}</a>
                    <a href="{{route('website.our.branches')}}"> {{__('website.footer.quick-links.link-2')}}</a>
                    <a href="{{route('website.galleries')}}"> {{__('website.footer.quick-links.link-3')}}</a>
                    <a href="{{route('website.reservation')}}"> {{__('website.footer.quick-links.link-4')}}</a>
                    <a href="{{route('website.questionnaire')}}"> {{__('website.footer.quick-links.link-5')}}</a></nav>
            </div>
            <div class="col-sm-2 boxfot">
                <h6 class="title"> {{__('website.footer.about-us.title')}}</h6>
                <nav class="navmenu">
                    <a href="{{route('website.about')}}">{{__('website.footer.about-us.link-1')}}</a>
                    <a href="{{route('website.our.branches')}}">{{__('website.footer.about-us.link-2')}}</a>
                    <a href="{{route('website.employment')}}">{{__('website.footer.about-us.link-6')}}</a>
                    <a href="{{route('website.terms-and-conditions')}}">{{__('website.footer.about-us.link-3')}}</a>
                    <a href="{{route('website.policy')}}">{{__('website.footer.about-us.link-4')}}</a>
                    <a href="{{route('website.contact')}}">{{__('website.footer.about-us.link-5')}}</a></nav>
            </div>
        </div>
    </div>
    <div class="item-bg"><img src="{{asset('/assets/website/images/bg.svg')}}" alt="" title=""></div>
    <div class="copyright">
        <div class="container">

            <p>{{__('website.copy-rights')}}</p>
            <nav class="social"><a href="https://www.facebook.com/alqalzamksa/?ref=bookmarks" target="_blank">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g clip-path="url(#clip0)">
                            <path d="M12.4997 3.33332H14.9997C15.2298 3.33332 15.4164 3.14676 15.4164 2.91664V0.416641C15.4164 0.186563 15.2298 0 14.9997 0H12.4997C9.96953 0.00277344 7.9191 2.05316 7.91637 4.58332V7.5H4.99969C4.76957 7.5 4.58301 7.68656 4.58301 7.91668V10.4167C4.58301 10.6468 4.76957 10.8334 4.99969 10.8334H7.91637V19.5834C7.91637 19.8135 8.10293 20 8.33305 20H10.833C11.0632 20 11.2497 19.8135 11.2497 19.5834V10.8334H14.1664C14.3457 10.8332 14.5048 10.7184 14.5614 10.5484L15.3947 8.04836C15.4675 7.83004 15.3494 7.5941 15.1311 7.52137C15.0888 7.50727 15.0444 7.50004 14.9997 7.5H11.2497V4.58332C11.2497 3.89297 11.8093 3.33332 12.4997 3.33332Z" fill="#656565"></path>
                        </g>
                    </svg></a><a href="https://twitter.com/alqalzamksa?lang=en" target="_blank">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g clip-path="url(#clip0)">
                            <path d="M20 3.79875C19.2563 4.125 18.4637 4.34125 17.6375 4.44625C18.4875 3.93875 19.1363 3.14125 19.4412 2.18C18.6488 2.6525 17.7738 2.98625 16.8412 3.1725C16.0887 2.37125 15.0162 1.875 13.8462 1.875C11.5763 1.875 9.74875 3.7175 9.74875 5.97625C9.74875 6.30125 9.77625 6.61375 9.84375 6.91125C6.435 6.745 3.41875 5.11125 1.3925 2.6225C1.03875 3.23625 0.83125 3.93875 0.83125 4.695C0.83125 6.115 1.5625 7.37375 2.6525 8.1025C1.99375 8.09 1.3475 7.89875 0.8 7.5975C0.8 7.61 0.8 7.62625 0.8 7.6425C0.8 9.635 2.22125 11.29 4.085 11.6712C3.75125 11.7625 3.3875 11.8062 3.01 11.8062C2.7475 11.8062 2.4825 11.7913 2.23375 11.7362C2.765 13.36 4.2725 14.5538 6.065 14.5925C4.67 15.6838 2.89875 16.3412 0.98125 16.3412C0.645 16.3412 0.3225 16.3263 0 16.285C1.81625 17.4563 3.96875 18.125 6.29 18.125C13.835 18.125 17.96 11.875 17.96 6.4575C17.96 6.27625 17.9538 6.10125 17.945 5.9275C18.7588 5.35 19.4425 4.62875 20 3.79875Z" fill="#656565"></path>
                        </g>
                    </svg></a><a href="https://www.instagram.com/alqalzamksa/channel/" target="_blank">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M14.4805 0H5.51913C2.47588 0 0 2.476 0 5.51924V14.4806C0 17.524 2.47588 19.9999 5.51913 19.9999H14.4805C17.524 19.9999 19.9999 17.5239 19.9999 14.4806V5.51924C20 2.476 17.524 0 14.4805 0ZM18.2255 14.4806C18.2255 16.5455 16.5455 18.2254 14.4806 18.2254H5.51913C3.45433 18.2255 1.77449 16.5455 1.77449 14.4806V5.51924C1.77449 3.45445 3.45433 1.77449 5.51913 1.77449H14.4805C16.5454 1.77449 18.2254 3.45445 18.2254 5.51924V14.4806H18.2255Z" fill="#656565"></path>
                        <path d="M10.0001 4.84668C7.15848 4.84668 4.84668 7.15848 4.84668 10.0001C4.84668 12.8417 7.15848 15.1534 10.0001 15.1534C12.8418 15.1534 15.1536 12.8417 15.1536 10.0001C15.1536 7.15848 12.8418 4.84668 10.0001 4.84668ZM10.0001 13.3788C8.13705 13.3788 6.62117 11.8631 6.62117 10C6.62117 8.13681 8.13693 6.62105 10.0001 6.62105C11.8634 6.62105 13.3791 8.13681 13.3791 10C13.3791 11.8631 11.8632 13.3788 10.0001 13.3788Z" fill="#656565"></path>
                        <path d="M15.3696 3.3418C15.0278 3.3418 14.6919 3.48021 14.4505 3.72272C14.2078 3.96405 14.0684 4.30002 14.0684 4.64309C14.0684 4.98509 14.208 5.32094 14.4505 5.56345C14.6918 5.80478 15.0278 5.94438 15.3696 5.94438C15.7127 5.94438 16.0475 5.80478 16.29 5.56345C16.5325 5.32094 16.6709 4.98497 16.6709 4.64309C16.6709 4.30002 16.5325 3.96405 16.29 3.72272C16.0487 3.48021 15.7127 3.3418 15.3696 3.3418Z" fill="#656565"></path>
                    </svg></a><a href="https://web.whatsapp.com/send?phone=966550021515&text=%D9%84%D8%B7%D9%84%D8%A8%D8%A7%D8%AA+%D8%A7%D9%84%D8%AA%D9%88%D8%B5%D9%8A%D9%84" target="_blank">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M16.6319 3.34007C14.8778 1.58386 12.5449 0.616264 10.0596 0.615234C4.93836 0.615234 0.770424 4.78306 0.768364 9.90566C0.767677 11.5432 1.19546 13.1417 2.00856 14.5507L0.69043 19.3652L5.61585 18.0732C6.97301 18.8135 8.50091 19.2036 10.0558 19.2041H10.0597C15.1804 19.2041 19.3488 15.0359 19.3507 9.9131C19.3517 7.43042 18.3862 5.09617 16.6319 3.34007ZM10.0596 17.635H10.0564C8.67074 17.6344 7.31175 17.262 6.12592 16.5586L5.84405 16.3911L2.92123 17.1579L3.70137 14.3082L3.51769 14.016C2.74464 12.7865 2.33643 11.3654 2.33712 9.90623C2.33872 5.64846 5.80308 2.18445 10.0627 2.18445C12.1254 2.18513 14.0643 2.98943 15.5223 4.44912C16.9803 5.90881 17.7828 7.84904 17.7821 9.91253C17.7802 14.1706 14.3161 17.635 10.0596 17.635ZM14.2955 11.8513C14.0634 11.735 12.922 11.1736 12.7091 11.096C12.4965 11.0185 12.3415 10.9799 12.1869 11.2122C12.0321 11.4445 11.5873 11.9675 11.4518 12.1224C11.3163 12.2773 11.181 12.2968 10.9488 12.1805C10.7166 12.0644 9.96861 11.8191 9.08181 11.0282C8.39173 10.4126 7.92584 9.6524 7.79034 9.42009C7.65508 9.18754 7.7892 9.07402 7.8922 8.9463C8.14351 8.63422 8.39516 8.30704 8.47253 8.1522C8.55 7.99724 8.51121 7.86163 8.45307 7.74547C8.39516 7.62932 7.93088 6.48662 7.73747 6.02165C7.54887 5.56915 7.35764 5.63026 7.21505 5.62317C7.07978 5.61642 6.92494 5.61504 6.7701 5.61504C6.61538 5.61504 6.36384 5.67306 6.15098 5.90561C5.93823 6.13804 5.33856 6.6996 5.33856 7.84229C5.33856 8.98498 6.17043 10.0889 6.28648 10.2438C6.40252 10.3988 7.92355 12.7437 10.2523 13.7492C10.8062 13.9886 11.2386 14.1313 11.5758 14.2383C12.132 14.415 12.6379 14.39 13.038 14.3303C13.4841 14.2636 14.4114 13.7686 14.6051 13.2264C14.7985 12.6841 14.7985 12.2193 14.7403 12.1224C14.6824 12.0256 14.5276 11.9675 14.2955 11.8513Z" fill="#656565"></path>
                    </svg></a><a href="#">
                    <svg width="17" height="20" viewBox="0 0 17 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M13.7567 10.6358C13.7337 8.10909 15.8508 6.87984 15.9476 6.82261C14.7486 5.09555 12.8902 4.85958 12.2371 4.84076C10.6762 4.67848 9.16217 5.76348 8.36706 5.76348C7.55607 5.76348 6.33165 4.85644 5.01201 4.8831C3.31386 4.90897 1.72521 5.88029 0.853914 7.38863C-0.944225 10.4641 0.396841 14.9836 2.11959 17.4696C2.98137 18.6871 3.98836 20.0464 5.30641 19.9986C6.5959 19.9461 7.07757 19.1864 8.63368 19.1864C10.1755 19.1864 10.6278 19.9986 11.9721 19.968C13.356 19.9461 14.2273 18.7451 15.0589 17.5166C16.0548 16.1212 16.4547 14.7469 16.4706 14.6763C16.438 14.6653 13.7829 13.6642 13.7567 10.6358Z" fill="#656565"></path>
                        <path d="M11.2171 3.20561C11.9107 2.34874 12.3852 1.183 12.2535 0C11.2497 0.0439017 9.99429 0.685965 9.27138 1.52402C8.6318 2.26251 8.06046 3.47294 8.20805 4.61125C9.33566 4.69435 10.4934 4.04915 11.2171 3.20561Z" fill="#656565"></path>
                    </svg></a><a href="#">
                    <svg width="19" height="20" viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M0.35757 0.42093C0.130914 0.660695 0 1.02706 0 1.50659V18.5145C0 18.9941 0.130913 19.3604 0.365386 19.5906L0.425958 19.6424L10.1331 10.1132V9.90027L0.418142 0.369141L0.35757 0.42093Z" fill="#656565"></path>
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M14.0277 13.2989L10.79 10.1206V9.89805L14.0277 6.71973L14.0981 6.76193L17.9278 8.89871C19.0239 9.50483 19.0239 10.5061 17.9278 11.1199L14.0981 13.2567L14.0277 13.2989Z" fill="#656565"></path>
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M13.7779 13.3528L10.4699 10.1055L0.702148 19.6941C1.05972 20.0701 1.65958 20.1123 2.32978 19.7459L13.7779 13.3528Z" fill="#656565"></path>
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M13.7779 6.6409L2.32978 0.257402C1.65958 -0.118549 1.05776 -0.0667596 0.702148 0.309192L10.4699 9.88828L13.7779 6.6409Z" fill="#656565"></path>
                    </svg></a></nav>
        </div>
    </div>
</footer>
