<div x-data class="flex flex-col">
    <div class="flex">
        ...
        <select
            x-ref="select"
            name="{{ $name }}"
            class="form-control m-5 text-sm leading-4 flex-grow form-select w-100px"
            wire:input="doSelectFilter('{{ $name }}', $event.target.value)"
            x-on:input="$refs.select.value=''"
        >
            <option value="">{{  __('main.select') }} {{ $name }}</option>
