<div class="modal-dialog">
    <div class="modal-content add-ad">
        ...
        <form action="#" method="post" wire:submit.prevent="updateAddress({{$addressId}})">
            @if(count($errors) > 0)
            ...
            <div class="field">
                ...
                <select class="form-control" name="addressLocationId" wire:model="location_id">
                    ...
                    @foreach($districts as $district)
                        <option value="{{$district->id}}" {{$location_id == $district->id ? 'selected' : ''}}>{{$district->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="field">
            ...
            <div class="field">
                <button class="bottom" type="submit">تعديل</button>
            </div>
        </form>
    </div>
</div>
