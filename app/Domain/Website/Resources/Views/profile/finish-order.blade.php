...
{{--                                                <h4 class="title">توصيل للموقع الحالي</h4></a></div>--}}
                                        @foreach(auth()->user()->addresses()->with('location')->get() as $address)
                                            <div class="col-sm-4 selection">
                                                <label class="che-box">
                                                    <input type="radio" data-address="{{$address}}" name="address_id" value="{{$address->id}}"><span class="label-text"></span>
                                                    <div class="text-chek">
                                                        <h4 class="title">السعوديه، {{$address->location->name}}</h4>
                                                        <p>{{$address->address_1}}</p>
                                                        <nav class="ac-link"><a href="#add-address" data-toggle="modal"><img src="images/edit.svg" alt="" title=""> تعديل</a><a href="#delete" data-toggle="modal"><img src="images/delete.svg" alt="" title=""> حذف</a></nav>
                                                    </div>
                                                </label>
                                            </div>
                                        @endforeach
    ...
    <div class="modal fade" id="completion" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content login">
                ...
                <p class="text">يرجي التأكيد، بأن طلبك من <span>فرع عابر  القارات</span> علي عنوان <span>جدة، الحي الأول، عمارة رقم ٣</span></p>
