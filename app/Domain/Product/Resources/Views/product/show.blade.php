                ...
                <div class="col-md-12">
                    <div class="card card-custom">
                        <div class="card-header">
                            <h3 class="card-title">
                                {{ __('main.show') }} {{ __('main.product') }} : # {{ $show->id }}
                            </h3>
                            <div class="card-toolbar">
                                <a title="{{ __('main.create') }} {{ __('main.product') }}"
                                   href="{{ route('products.create') }}"
                                   class="btn btn-light-primary font-weight-bolder mr-2">
                                    <i class="flaticon2-plus"
                                       style="color: #FFF"></i> {{ __("main.add") }} {{ __("main.product") }} </a>
                                <a title="{{ __('main.delete') }} {{ __('main.product') }}"
                                   href="{{ route('products.destroy', $show->id) }}"
                                   class="btn btn-light-danger font-weight-bolder mr-2" data-toggle="modal"
                                   data-target="#delete_{{$show->id}}">
                                    <i class="flaticon2-trash"></i> {{ __("main.delete") }} {{ __("main.product") }} </a>
                                <a title="{{ __('main.edit') }} {{ __('main.product') }}"
                                   href="{{ route('products.edit', $show->id) }}"
                                   class="btn btn-light-warning font-weight-bolder mr-2">
                                    <i class="flaticon2-edit"></i> {{ __("main.edit") }} {{ __("main.product") }} </a>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="row mb-2">
                                        ...
                                        @if($show->getFirstMedia('product-images'))
                                        <div id="lightgallery">
                                            <div data-src="{{ $show->getFirstMedia('product-images')->getUrl() }}">
                                                <a href="{{ $show->getFirstMedia('product-images')->getUrl() }}">
                                                    <img class="d-block" src="{{ $show->getFirstMedia('product-images')->getUrl() }}"
                                                         style="width:100%" alt="...">
                                                </a>
                                            </div>
                                        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('main.delete') }} {{ __('main.product') }}
                        : {{ __('main.products') }} #({{ $show->name }})</h5>
                ...
                <form action="{{ route('products.destroy', $show->id) }}" method="post">
                    ...
                    <div class="modal-body">
                        {{ __('main.delete') }} {{ __('main.products') }}: {{ __('main.products') }} #({{ $show->name }})
                    </div>
