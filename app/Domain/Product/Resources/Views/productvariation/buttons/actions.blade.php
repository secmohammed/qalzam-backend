<div class="flex space-x-1">
    <a href="{{ route('product_variations.show', [$id]) }}" class="p-1 text-teal-600 hover:bg-teal-600 hover:text-white rounded">
    ...
    <a href="{{ route('product_variations.edit', [$id]) }}" class="p-1 text-blue-600 hover:bg-blue-600 hover:text-white rounded">
        <span class="svg-icon svg-icon-1.5x">
            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path
    ...
    <a href="{{ route('product_variations.destroy', [$id]) }}" class="p-1 text-red-600 hover:bg-red-600 hover:text-white rounded"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ __('main.delete') }} {{ __('main.product_variations') }}:
                    #{{ $id }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                </button>
            </div>
            <form action="{{ route('product_variations.destroy', [$id]) }}" method="post">
                @csrf
                @method("DELETE")
                <div class="modal-body">
                    {{ __('main.delete') }} {{ __('main.product_variations') }}: #{{ $id }}
                </div>
