            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{ route('stocks.index') }}" class="text-muted">
                                {{ __('main.stocks') }} </a>
                        </li>
                    </ul>
                    <!--end::Breadcrumb-->
                ...
                <div class="col-md-12">
                    <div class="card card-custom">
                        <div class="card-header">
                            <h3 class="card-title">
                                {{ __('main.show') }} {{ __('main.stock') }} : # {{ $show->id }}
                            </h3>
                            <div class="card-toolbar">
                                <a title="{{ __('main.create') }} {{ __('main.stock') }}"
                                   href="{{ route('stocks.create') }}"
                                   class="btn btn-light-primary font-weight-bolder mr-2">
                                    <i class="flaticon2-plus"
                                       style="color: #FFF"></i> {{ __("main.add") }} {{ __("main.stock") }} </a>
                                <a title="{{ __('main.delete') }} {{ __('main.stock') }}"
                                   href="{{ route('stocks.destroy', $show->id) }}"
                                   class="btn btn-light-danger font-weight-bolder mr-2" data-toggle="modal"
                                   data-target="#delete_{{$show->id}}">
                                    <i class="flaticon2-trash"></i> {{ __("main.delete") }} {{ __("main.stock") }} </a>
                                <a title="{{ __('main.edit') }} {{ __('main.stock') }}"
                                   href="{{ route('stocks.edit', $show->id) }}"
                                   class="btn btn-light-warning font-weight-bolder mr-2">
                                    <i class="flaticon2-edit"></i> {{ __("main.edit") }} {{ __("main.stock") }} </a>
                            </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 mb-5">
                                    <div class="row mb-2">
                                        <strong class='ml-3'><span>{{ __("main.product_variation") }} : </span></strong>
                                        <span>{{ $show->variation->name ?? 'N/A' }} </span>
                                    </div>
                                    <hr>
                                </div>
                                ...
                                <div class="col-md-6 mb-5">
                                    <div class="row mb-2">
                                        <strong class='ml-3'><span>{{ __("main.quantity") }} : </span></strong>
                                        <span>{{ $show->quantity ?? 'N/A' }} </span>
                                    </div>
                                    <hr>
                                </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('main.delete') }} {{ __('main.stock') }}
                        : {{ __('main.stocks') }} #({{ $show->name }})</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    </button>
                </div>
                <form action="{{ route('stocks.destroy', $show->id) }}" method="post">
                    @csrf
                    @method("DELETE")
                    <div class="modal-body">
                        {{ __('main.delete') }} {{ __('main.stocks') }}: {{ __('main.stocks') }} #({{ $show->name }})
                    </div>
