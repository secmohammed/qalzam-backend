...
{
    /** @test */
    public function it_should_fetch_accommodations_which_have_contract_that_contains_a_specific_day()
    {
        ...
        $anotherContract = Contract::factory()->create([
            'days' => [
                'sunday', 'friday',
            ],
        ]);
        ...
        $response = $this->jsonAs(
            $this->user,
            'GET',
            route('api.accommodations.index') . '?filter[accommodation_by_contract_containing_day]=monday&include=contract'
        );
    ...
    /** @test */
    public function it_should_fetch_accommodations_with_contract_when_available()
    {
        $this->accommodationFactory->withRoomContract()->create([

        ]);
        $response = $this->jsonAs(
            $this->user,
            'GET',
            route('api.accommodations.index') . '?include=contract'
        );
        $this->assertTrue(array_key_exists('contract', $response->getData(true)['data'][0]));
    ...
    public function it_should_filter_accommodations_by_capacity()
    {
        ...
        $response = $this->jsonAs(
            $this->user,
            'GET',
            sprintf('%s?%s=%s', route('api.accommodations.index'), 'filter[capacity]', $accommodation->capacity)
        )->assertJsonStructure([
            'data',
        ]);
