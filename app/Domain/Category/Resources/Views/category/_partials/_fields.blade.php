...
{{ __("main.type") }}</label>
    <div class="col-lg-10 col-md-9 col-sm-12">
        ...
        <select class="form-control select2 {{$errors->has('type') ? 'is-invalid':''}}" name="type" data-placeholder="{{ __('main.select') .' '.__('main.type')  }}">
            ...
            <option
                value="store" {{ ($action == 'edit') && $edit->type == 'store' ? 'selected' : '' }}>{{ __('main.store') }}</option>
            <option
                value="speciality" {{ ($action == 'edit') && $edit->type == 'speciality' ? 'selected' : '' }}>{{ __('main.speciality') }}</option>
