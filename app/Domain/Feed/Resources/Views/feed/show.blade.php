            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{ route('feeds.index') }}" class="text-muted">
                                {{ __('main.feeds') }} </a>
                        </li>
                    </ul>
                    <!--end::Breadcrumb-->
                ...
                <div class="col-md-12">
                    <div class="card card-custom">
                        <div class="card-header">
                            <h3 class="card-title">
                                {{ __('main.show') }} {{ __('main.feed') }} : # {{ $show->id }}
                            </h3>
                            <div class="card-toolbar">
                                <a title="{{ __('main.create') }} {{ __('main.feed') }}"
                                   href="{{ route('feeds.create') }}"
                                   class="btn btn-light-primary font-weight-bolder mr-2">
                                    <i class="flaticon2-plus"
                                       style="color: #FFF"></i> {{ __("main.add") }} {{ __("main.feed") }} </a>
                                <a title="{{ __('main.delete') }} {{ __('main.feed') }}"
                                   href="{{ route('feeds.destroy', $show->id) }}"
                                   class="btn btn-light-danger font-weight-bolder mr-2" data-toggle="modal"
                                   data-target="#delete_{{$show->id}}">
                                    <i class="flaticon2-trash"></i> {{ __("main.delete") }} {{ __("main.feed") }} </a>
                                <a title="{{ __('main.edit') }} {{ __('main.feed') }}"
                                   href="{{ route('feeds.edit', $show->id) }}"
                                   class="btn btn-light-warning font-weight-bolder mr-2">
                                    <i class="flaticon2-edit"></i> {{ __("main.edit") }} {{ __("main.feed") }} </a>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-5">
                                    <div class="row mb-2">
                                        ...
                                        <span>{{ $show->competition ? $show->competition->name : 'N/A' }} </span>
                                    </div>
                                    <hr>
                                </div>
                                <div class="col-md-6 mb-5">
                                    <div class="row mb-2">
                                        <strong class='ml-3'><span>{{ __("main.child") }} : </span></strong>
                                        <span>{{ $show->child ? $show->child->name : 'N/A' }} </span>
                                    </div>
                                    <hr>
                                </div>
                                <div class="col-md-6 mb-5">
                                ...
                                <div class="col-md-12">
                                    <div class="row mb-2">
                                        <strong class='ml-3'><span>{{ __("main.feed_media") }} : </span></strong>
                                        @if($show->getFirstMedia('feed-isomorphic'))
                                            <div id="lightgallery">
                                                <div data-src="{{ $show->getFirstMedia('feed-isomorphic')->getUrl() }}">
                                                    <a href="{{ $show->getFirstMedia('feed-isomorphic')->getUrl() }}">
                                                        <img class="d-block" src="{{ $show->getFirstMedia('feed-isomorphic')->getUrl() }}"
                                                             style="width:100%" alt="...">
                                                    </a>
                                                </div>
                                            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('main.delete') }} {{ __('main.feed') }}
                        : {{ __('main.feeds') }} #({{ $show->name }})</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    </button>
                </div>
                <form action="{{ route('feeds.destroy', $show->id) }}" method="post">
                    @csrf
                    @method("DELETE")
                    <div class="modal-body">
                        {{ __('main.delete') }} {{ __('main.feeds') }}: {{ __('main.feeds') }} #({{ $show->name }})
                    </div>
