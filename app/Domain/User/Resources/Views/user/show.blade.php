                ...
                <div class="col-md-12">
                    <div class="card card-custom">
                        <div class="card-header">
                            <h3 class="card-title">
                                {{ __('main.show') }} {{ __('main.user') }} : # {{ $show->id }}
                            </h3>
                            <div class="card-toolbar">
                                <a title="{{ __('main.create') }} {{ __('main.user') }}"
                                   href="{{ route('users.create') }}"
                                   class="btn btn-light-primary font-weight-bolder mr-2">
                                    <i class="flaticon2-plus"
                                       style="color: #FFF"></i> {{ __("main.add") }} {{ __("main.user") }} </a>
                                <a title="{{ __('main.delete') }} {{ __('main.user') }}"
                                   href="{{ route('users.destroy', $show->id) }}"
                                   class="btn btn-light-danger font-weight-bolder mr-2" data-toggle="modal"
                                   data-target="#delete_{{$show->id}}">
                                    <i class="flaticon2-trash"></i> {{ __("main.delete") }} {{ __("main.user") }} </a>
                                <a title="{{ __('main.edit') }} {{ __('main.user') }}"
                                   href="{{ route('users.edit', $show->id) }}"
                                   class="btn btn-light-warning font-weight-bolder mr-2">
                                    <i class="flaticon2-edit"></i> {{ __("main.edit") }} {{ __("main.user") }} </a>
                            </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-5">
                                    <div class="row mb-2">
                                        <strong class='ml-3'><span>{{ __("main.email") }} : </span></strong>
                                        <span>{{ $show->email ?? 'N/A' }} </span>
                                    </div>
                                    <hr>
                                </div>
                                <div class="col-md-6 mb-5">
                                ...
                                <div class="col-md-12">
                                    <div class="row mb-2">
                                        ...
                                        @if($show->getFirstMedia('image'))
                                        <div id="lightgallery">
                                            <div data-src="{{ $show->getFirstMedia('image')->getUrl() }}">
                                                <a href="{{ $show->getFirstMedia('image')->getUrl() }}">
                                                    <img class="d-block" src="{{ $show->getFirstMedia('image')->getUrl() }}"
                                                         style="width:100%" alt="...">
                                                </a>
                                            </div>
                                        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('main.delete') }} {{ __('main.user') }}
                        : {{ __('main.users') }} #({{ $show->name }})</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    </button>
                </div>
                <form action="{{ route('users.destroy', $show->id) }}" method="post">
                    @csrf
                    @method("DELETE")
                    <div class="modal-body">
                        {{ __('main.delete') }} {{ __('main.users') }}: {{ __('main.users') }} #({{ $show->name }})
                    </div>
