                ...
                <div class="col-md-12">
                    <div class="card card-custom">
                        <div class="card-header">
                            <h3 class="card-title">
                                {{ __('main.show') }} {{ __('main.role') }} : # {{ $show->id }}
                            </h3>
                            <div class="card-toolbar">
                                <a title="{{ __('main.create') }} {{ __('main.role') }}"
                                   href="{{ route('roles.create') }}"
                                   class="btn btn-light-primary font-weight-bolder mr-2">
                                    <i class="flaticon2-plus"
                                       style="color: #FFF"></i> {{ __("main.add") }} {{ __("main.role") }} </a>
                                <a title="{{ __('main.delete') }} {{ __('main.role') }}"
                                   href="{{ route('roles.destroy', $show->id) }}"
                                   class="btn btn-light-danger font-weight-bolder mr-2" data-toggle="modal"
                                   data-target="#delete_{{$show->id}}">
                                    <i class="flaticon2-trash"></i> {{ __("main.delete") }} {{ __("main.role") }} </a>
                                <a title="{{ __('main.edit') }} {{ __('main.role') }}"
                                   href="{{ route('roles.edit', $show->id) }}"
                                   class="btn btn-light-warning font-weight-bolder mr-2">
                                    <i class="flaticon2-edit"></i> {{ __("main.edit") }} {{ __("main.role") }} </a>
                            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('main.delete') }} {{ __('main.role') }}
                        : {{ __('main.roles') }} #({{ $show->name }})</h5>
                ...
                <form action="{{ route('roles.destroy', $show->id) }}" method="post">
                    ...
                    <div class="modal-body">
                        {{ __('main.delete') }} {{ __('main.roles') }}: {{ __('main.roles') }} #({{ $show->name }})
                    </div>
