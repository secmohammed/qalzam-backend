<div class="flex space-x-1">
    <a href="{{ route('branch_shifts.show', [$id]) }}"
    ...
    <a href="{{ route('branch_shifts.edit', [$id]) }}"
    ...
    <a href="{{ route('branch_shifts.destroy', [$id]) }}"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ __('main.delete') }} {{ __('main.branch_shifts') }}:
                    #{{ $id }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                </button>
            </div>
            <form action="{{ route('branch_shifts.destroy', [$id]) }}" method="post">
                @csrf
                @method("DELETE")
                <div class="modal-body">
                    {{ __('main.delete') }} {{ __('main.branch_shifts') }}: #{{ $id }}
                </div>
